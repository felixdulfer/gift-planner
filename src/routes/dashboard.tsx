...
function DashboardPage() {
	...
	usePersistCollection(giftsStore, 'gifts', gifts.data as GiftType[] | undefined)
	usePersistCollection(
		giftAssignmentsStore,
		'giftAssignments',
	...
	const myAssignments = (giftAssignments.data as GiftAssignment[] | undefined)?.filter(
		(assignment) =>
	...
	const allGiftAssignments = (giftAssignments.data as GiftAssignment[] | undefined) ?? []
	const qualifiedGifts = (gifts.data as GiftType[] | undefined)?.filter(
	...
	const qualifiedGiftsWithContext = qualifiedGifts
		.map((gift) => {
			...
			const assignment = allGiftAssignments.find((a) => a.giftId === gift.id)
			if (
				assignment &&
				assignment.isPurchased &&
				...
				<div className="container mx-auto py-8 px-4 max-w-6xl">
					<div className="mb-8">
						...
						<p className="text-muted-foreground mt-2">
							View qualified gifts, assigned gifts, and upcoming events
						</p>
					</div>
					...
					{sortedQualifiedGifts.length > 0 && (
						<div className="mb-12">
							...
							<p className="text-muted-foreground mb-4">
								These gifts from wishlists need to be bought but haven't been
								assigned yet
							</p>
								...
								{sortedAssignedGifts.map((item) => (
									<AssignedGiftCard
										...
										onMarkPurchased={(giftId) => {
											setRecentlyPurchasedGiftIds((prev) => {
